...
# for each line, calculate the sequence length
		for line in dna_file:
			...
			for bin_lower in range(100, 1000, 100):  # checking/creating criteria for the bins
				...
				if length >= bin_lower and length < bin_upper: #cannot be = to upper_bin, because upper bin, starts new directory
					...
					bin_filename = str(bin_lower) + "_" + str(bin_upper) + '.dnaout'
					output = open(bin_filename, "a")
					output.write(dna + "\n")
					output.close()
